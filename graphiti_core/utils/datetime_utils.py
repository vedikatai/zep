...
from graphiti_core.utils.replay import get_replay_context


def utc_now() -> datetime:
    """Returns the current UTC datetime with timezone information.

    When a ReplayContext is active, returns its fixed_timestamp instead of
    reading the system clock so episode replay is deterministic.
    """
    ctx = get_replay_context()
    if ctx is not None:
        return ctx.fixed_timestamp
